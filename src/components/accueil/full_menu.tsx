    ...
    return (
        <div className="absolute top-0 left-0 right-0 bg-[#0d0d0d] z-50">
            ...
            <div className="flex flex-col justify-around items-center ml-[50%] w-40 h-[100vh] pt-40 pb-20">
                <span className={" text-white absolute top-0 right-28 top-14 z-51 cursor-pointer text-3xl"} onClick={() => {
                    props.onCloseMenuPage()
                }}><MdClose/></span>
                {
                    menuItems.map((item, i) => (
                        <Link href={item.href} key={i} className={" cursor-pointer text-white text-3xl flex flex-row justify-start w-20"}>
